 ...
 * @param maxPower The maximum power the DriveTrain may sent to each of its motors. Capped at 1.0.
...
public record DriveTrainAutoModule(double targetX, double targetY, double targetAngle, double posTolerance, double angleTolerance, double maxPower, boolean usePrecision, double weight) {
    public Pose targetPose() { return new Pose(new Point(targetX, targetY), new Angle(targetAngle)); }
}